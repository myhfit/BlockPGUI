...
public class BPTabBottom extends BPTabbedContainerBase
{
	...
	protected void initUI()
	{
		...
		addBPTab("TasksUI", (Icon) null, BPActionHelpers.getValue(BPActionConstCommon.TXT_TASK), m_tasksui);
		...
		if ("true".equalsIgnoreCase(BPEnvManager.getEnvValue(BPEnvCommon.ENV_NAME_COMMON, BPEnvCommon.ENVKEY_ENABLE_SCHEDULE)))
		{
			...
			addBPTab("SchedulesUI", null, BPActionHelpers.getValue(BPActionConstCommon.TXT_SCHEDULE), m_sdsui, false);
		}
		BPConsoleUI consoleui = new BPConsoleUI();
		addBPTab("ConsoleUI", null, BPActionHelpers.getValue(BPActionConstCommon.TXT_CONSOLE), consoleui, false);
		addBPTab("TasksUIWL", null, BPActionHelpers.getValue(BPActionConstCommon.TXT_WORKING), m_tasksuiwl, false);
