...
public class BPTabBottom extends BPTabbedContainerBase
{
	...
	protected void initUI()
	{
		...
		addBPTab("TasksUI", (Icon) null, BPActionConstCommon.TXT_TASK.text(), m_tasksui);
		...
		if ("true".equalsIgnoreCase(BPEnvManager.getEnvValue(BPEnvCommon.ENV_NAME_COMMON, BPEnvCommon.ENVKEY_ENABLE_SCHEDULE)))
		{
			...
			addBPTab("SchedulesUI", null, BPActionConstCommon.TXT_SCHEDULE.text(), m_sdsui, false);
		}
		BPConsoleUI consoleui = new BPConsoleUI();
		addBPTab("ConsoleUI", null, BPActionConstCommon.TXT_CONSOLE.text(), consoleui, false);
		addBPTab("TasksUIWL", null, BPActionConstCommon.TXT_WORKING.text(), m_tasksuiwl, false);
